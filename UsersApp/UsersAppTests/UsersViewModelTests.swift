...
class UsersViewModelTests: XCTestCase {
    ...
    func testInitialization() {
        XCTAssertTrue(viewModel.APIManager === successAPI, "The API should be equal to the API that was passed in.")
    }
    ...
    func testUsersAddFavorite() {
        viewModel.addFavorited(view: usersView, user: .fixture(), indexPath: .init(row: 1, section: 1))
    ...
    func testUsersRemoveFavorite() {
        ...
        viewModel.addFavorited(view: usersView, user: fakeUser, indexPath: .init(row: 1, section: 1))
        
        XCTAssertTrue(coreDataMock.loadedFavoriteUsers[0].id == fakeUser.id)
        
        viewModel.removeFavorited(view: usersView, user: fakeUser, indexPath: .init(row: 1, section: 1))
